...
#define CKKBH                           216
#define CKICOCK                         35
#define CKCHAR_CORNER                   8
#define CKKBFontSize                    18
//#define CKKBFont(s)                     [UIFont fontWithName:@"HelveticaNeue-Light" size:s]
#define CKKBFont(s)                     [UIFont boldSystemFontOfSize:s]

#define CKFormat(format, ...)           [NSString stringWithFormat:format, ##__VA_ARGS__]
#define CKSCREEN_WIDTH                  [UIScreen mainScreen].bounds.size.width
#define CKSCREEN_HEIGHT                 [UIScreen mainScreen].bounds.size.height
#define CKRGBHex(rgbValue) [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]

#define allBgColor CKRGBHex(0xdbdbdb)
#define titleColor CKRGBHex(0x444444)
#define touchColor CKRGBHex(0xFFFFFF)
#define bgColor CKRGBHex(0xcbcbcb)
#define wtColor CKRGBHex(0xffffff)
...
@interface CKKeyboard ()

@property (nonatomic, assign) CKBType type;

@property (nonatomic, assign) BOOL shiftEnable,showSymbol,showMoreSymbol;
@property (nonatomic, strong) NSMutableArray *charsBtn;

@property (nonatomic, strong) UIButton *shiftBtn,*deleteBtn,*charSymSwitch,*spaceBtn;
...
@implementation CKKeyboard

#pragma mark - Life Cycle

- (nonnull instancetype)initWithType:(CKBType)type
{
    self = [super initWithFrame:CGRectMake(0, 0, CKSCREEN_WIDTH, CKKBH+CKICOCK)];
    if (self) {
...
- (void)initSetup
{
    ...
    //    self.icon = @"58";
    self.enterprise = @"华融消费金融安全输入";
    
    CGRect bounds = self.bounds;
    bounds.size.height = iPhoneX ?CKKBH+CKICOCK +19:CKKBH+CKICOCK;
    
    self.bounds = bounds;
    self.backgroundColor = allBgColor;
    //seperate line
    CGFloat lineH = 1;
    bounds = CGRectMake(0, CKICOCK-lineH, CKSCREEN_WIDTH,lineH);
    UIView *line = [[UIView alloc] initWithFrame:bounds];
    line.backgroundColor = bgColor;
    [self addSubview:line];
    
    //创建键盘
    if (CKBTypeNumberPad == self.type) {
        
        [self reloadNumberPad:false];
        
    }else if (CKBTypeDecimalPad == self.type){
        
        [self reloadNumberPad:true];
        
    }else if (CKBTypeASCIICapable == self.type){
...
- (void)reloadNumberPad:(BOOL)decimal {
    if (self.type != CKBTypeASCIICapable) {
        
        int cols = 3;
        int rows = 4;
        
        
        UIFont *titleFont = CKKBFont(CKKBFontSize);
        CGFloat itemH = CKKBH/rows;
        CGFloat itemW = CKSCREEN_WIDTH/cols;
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                CGRect bounds = CGRectMake(j*itemW, i*itemH+CKICOCK, itemW, itemH);
                ...
                btn.layer.borderColor = allBgColor.CGColor;
                btn.frame = bounds;
...
- (void)loadRandomNumber {
    BOOL decimal = (self.type == CKBTypeDecimalPad);
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[UIButton class]] && obj!= self.packUpButton) {
...
#pragma mark 返回所有可变数字加上符号
- (NSArray *)getRandomNumberSymbolsArrays
{
    NSMutableArray *allMuArray = [NSMutableArray array];
    [allMuArray addObjectsFromArray:[self generateRandomNumberWithDecimal:YES]];
    [allMuArray addObjectsFromArray:Symbols];
    return allMuArray;
}
...
- (void)remoAllView
{
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[CKChar class]] || ([obj isKindOfClass:[UIButton class]]&& (obj != self.packUpButton))) {
...
- (void)setupASCIICapableLayout:(BOOL)init {
    ...
    //    if (!self.showSymbol && !self.showMoreSymbol) {
    //
    //        [self remoAllView];
    //        // 开始数字键盘布局
    //        self.type = CKBTypeDecimalPad;
    //        [self reloadNumberPad:true];
    //
    //        return;
    //    }
    ...
    if (!init) {
        ...
        [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[CKChar class]]) {
                [obj removeFromSuperview];
            }
        }];
    }
    ...
    if (!self.showSymbol) {
        ...
        charSets = self.showMoreSymbol? moreSymbols:[self getRandomNumberSymbolsArrays];
        rangs = @[@10,@20,@25];
    }
    ...
    CGFloat char_width = (CKSCREEN_WIDTH-char_h_dis*len)/len;
    CGFloat char_heigh = (CKKBH-char_uper_dis*2-char_v_dis*3)/4;
    UIFont *titleFont = CKKBFont(CKKBFontSize);
    //    UIColor *titleColor = CKColor(247, 247, 247);
    //    UIColor *bgColor = CKColor(64, 66, 68);
    UIImage *bgImg = [UIImage pb_imageFromColor:wtColor];
    CGFloat cur_y = CKICOCK+char_uper_dis;
    
    int n = 0;
    UIImage *charbgImg = [bgImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    for (int i = 0 ; i < len; i ++) {
        ...
        CKChar *btn = [CKChar buttonWithType:UIButtonTypeSystem];
        btn.frame = bounds;
        //btn.chars = title;
        btn.exclusiveTouch = true;
        //消耗性能
    ...
    //// NSLog(@"第二排:%@",chars);
    len = [chars count];
    CGFloat start_x = (CKSCREEN_WIDTH-char_width*len-char_h_dis*(len-1))/2;
    for (int i = 0 ; i < len; i ++) {
        ...
        btn.exclusiveTouch = true;
        //[btn addRoundCornerBackdround];
        //btn.backgroundColor = CKColor(64, 66, 68);
        btn.userInteractionEnabled = false;
        btn.titleLabel.font = titleFont;
        btn.titleLabel.textAlignment = NSTextAlignmentCenter;
        btn.titleLabel.textColor = titleColor;
    ...
    char_width = (CKSCREEN_WIDTH-char_h_dis*4-shiftWidth*2-char_h_dis*(len-1))/len;
    charbgImg = [bgImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    CGRect bounds;UIButton *btn;
    if (init) {
        UIImage *roundImg = [bgImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:CGSizeMake(shiftWidth, char_heigh)];
        ...
        bounds = CGRectMake(CKSCREEN_WIDTH-char_h_dis*0.5-shiftWidth, cur_y, shiftWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeSystem];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = CKColor(64, 66, 68);
    ...
    if (init) {
        ...
        UIImage *grayImg = [UIImage pb_imageFromColor:bgColor];
        
        UIImage *roundImg = [grayImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:CGSizeMake(numWidth, char_heigh)];
        ...
        bounds = CGRectMake(CKSCREEN_WIDTH-char_h_dis*0.5-numWidth, cur_y, numWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeSystem];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = CKColor(64, 66, 68);
        ...
        CGFloat spaceWidth = (CKSCREEN_WIDTH-char_h_dis*3-numWidth*2);
        bounds = CGRectMake(char_h_dis*1.5+numWidth, cur_y, spaceWidth, char_heigh);
        
        UIImage *spaceImg = [bgImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:bounds.size];
        ...
        [btn setBackgroundImage:spaceImg forState:UIControlStateNormal];
...
- (void)loadCharacters:(NSArray *)array {
    ...
    NSArray *subviews = self.subviews;
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj && [obj isKindOfClass:[CKChar class]]) {
            CKChar *tmp = (CKChar *)obj;
            NSInteger __tag = tmp.tag;
...
- (void)shiftAction:(UIButton *)btn {
    ...
    if (self.showSymbol) {
        ...
        NSArray *__symbols = self.showMoreSymbol?moreSymbols:[self getRandomNumberSymbolsArrays];
...
- (void)characterTouchAction:(CKChar *)btn {
    NSString *title = [btn titleLabel].text;
    if (self.inputSource) {
        if ([self.inputSource isKindOfClass:[UITextField class]]) {
            UITextField *tmp = (UITextField *)self.inputSource;
...
- (void)touchDownAction:(UIButton *)btn {
    //    UIColor *touchColor = CKColor(43, 116, 224);
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (CKChar *tmp in self.charsBtn) {
...
- (CGFloat)widthForInfo:(NSString *)info {
    ...
    NSDictionary *attributs = [NSDictionary dictionaryWithObjectsAndKeys:CKKBFont(CKKBFontSize-5), NSFontAttributeName, nil];
...
- (void)setEnterprise:(NSString *)enterprise {
    if (enterprise.length > 0) {
        ...
        CGFloat iconH = CKICOCK;
        CGFloat icon_w_h = iconH*0.5;
        CGFloat width = [self widthForInfo:enterprise];
        //        CGFloat wwww = icon_w_h*1.5+width;
        //        CGFloat start_x = (CKSCREEN_WIDTH-wwww)*0.5+icon_w_h*1.5;
        CGFloat start_x= (self.width - width)*0.5;
        CGRect bounds = CGRectMake(start_x, CKICOCK*0.25, width, CKICOCK*0.5);
        ...
        self.packUpButton.frame = CGRectMake(CKSCREEN_WIDTH - 60, 0, 60, CKICOCK-1);
    }
}
...
- (UILabel *)iconLabel {
    if (!_iconLabel) {
        ...
        _iconLabel.font = CKKBFont(CKKBFontSize-5);
...
- (UIImageView *)iconFlag {
    if (!_iconFlag) {
        ...
        _iconFlag.layer.cornerRadius = CKICOCK*0.25;
...
{
    if (!_packUpButton) {
        _packUpButton = [UIButton buttonWithType:UIButtonTypeSystem];
        
        [_packUpButton setTitle:@"收起" forState:UIControlStateNormal];
        
        //        [_packUpButton setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
        
        _packUpButton.backgroundColor = [UIColor clearColor];
        
        _packUpButton.titleLabel.font = [UIFont boldSystemFontOfSize:13];
        
        [_packUpButton addTarget:self action:@selector(doneAction:) forControlEvents:UIControlEventTouchUpInside];
        // FIXME: 暂时去掉
...
@implementation CKChar

- (void)addRoundCornerBackdround {
    CGSize size = [self bounds].size;
    UIImage *backImg = [UIImage pb_imageFromColor:bgColor];
    backImg = [backImg pb_drawRectWithRoundCorner:CKCHAR_CORNER toSize:size];
...
- (void)addPopup {
    ...
    //        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CKKBImageRight] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
    ...
    //        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CKKBImageLeft] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
    //        keyPop.frame = CGRectMake(-38, -71, keyPop.frame.size.width, keyPop.frame.size.height);
    //    }
    //    else {
    //        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CKKBImageInner] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
    ...
    NSString *tmp = self.isShift?[self.chars uppercaseString]:[self.chars lowercaseString];
    [text setFont:[UIFont fontWithName:CKKBFont(CKKBFontSize).fontName size:44]];
